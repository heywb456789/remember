...
public class FamilyInfoResponseDTO {
     ...
     * 조회 모드용 DTO 생성 (답변 포함)
     */
    public static FamilyInfoResponseDTO forView(
        Memorial memorial,
        FamilyMember familyMember,
        List<MemorialQuestion> familQuestions,
        List<MemorialAnswer> answers,
        Integer completionPercent) {

        // Map each question ID to its answer (if any)
        Map<Long, MemorialAnswer> answerMap = answers.stream()
        ...
        // Build list of question+answer DTOs for all questions
        List<FamilyQuestionAnswerDTO> questionAnswers = familQuestions.stream()
            .map(q -> {
                MemorialAnswer ans = answerMap.get(q.getId());
                return FamilyQuestionAnswerDTO.builder()
                    .questionId(q.getId())
                    .questionText(q.getQuestionText())
                    .answerText(ans != null ? ans.getAnswerText() : null)
                    .answerLength(ans != null ? ans.getAnswerLength() : 0)
                    .isComplete(ans != null ? ans.getIsComplete() : false)
                    .build();
            })
     ...
     * 영상통화 차단 사유 반환
     */
    private static String getVideoCallBlockReason(Memorial memorial, FamilyMember familyMember,
        boolean canStartVideoCall) {
